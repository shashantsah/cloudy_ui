...
// Mock AiTextArea component
const AiTextArea = ({ onSubmit, onCancel, secondaryButtonText, onSecondaryAction, submitButtonText, existingLinkedFiles, showConnectTooltip, addButtonText }) => {
  return (
    <div className="flex flex-col gap-2 w-full">
      <textarea
        className="w-full resize-none border border-gray-200 rounded-md p-2"
      ...
      <div className="flex gap-2">
        <button
          ...
          onClick={() => onSubmit("Sample query", existingLinkedFiles)}
        >
          {submitButtonText}
        </button>
        {onSecondaryAction && (
          <button
            ...
            onClick={() => onSecondaryAction("Sample query", existingLinkedFiles)}
          >
            {secondaryButtonText}
          </button>
        )}
        <button className="bg-gray-200 text-gray-800 px-3 py-1 rounded-md" onClick={() => onCancel()}>
          Cancel
        </button>
      </div>
      {showConnectTooltip && <span className="text-sm text-gray-500">Connect files with {addButtonText}</span>}
    </div>
  );
};

// Mock existing linked files
const mockExistingLinkedFiles = [
  {
    repoConnectionId: "repo-1",
    fileName: "example.txt",
    repoFullName: "example-owner/example-repo",
    branch: "main",
    fileUrl: "https://github.com/example-owner/example-repo/blob/main/example.txt",
    path: "example.txt",
  },
];

export const AiSelectionMenu = ({ onCancel, onClose, thoughtContext }) => {
  const { editor, threadId, setThreadId, thoughtId, showAiEditor } = thoughtContext;

  const handleSubmit = async (text: string, fileReferences: any[]) => {
    const thread = { id: "new-thread-1" }; // Mock thread creation
    onClose();
    showAiEditor();
    setThreadId(thread.id);
  };

  const handleReply = async (text: string, fileReferences: any[]) => {
    if (!threadId) {
      throw new Error("Thread ID is not set");
    }
    onClose();
  ...
  return (
    <div className="z-50 flex w-[32rem] flex-col gap-0.5 rounded-md border border-gray-200 bg-gray-100 px-2 py-2 absolute top-20 left-10">
      <div className="flex flex-row items-center gap-1 pb-1 pl-2 pt-1">
